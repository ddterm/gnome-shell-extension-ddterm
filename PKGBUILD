...
build() {
    ...
    LIBGL_ALWAYS_SOFTWARE=1 GDK_BACKEND=x11 xvfb-run -- meson compile -C build
}

check() {
    LIBGL_ALWAYS_SOFTWARE=1 GDK_BACKEND=x11 xvfb-run -- meson test -C build --print-errorlogs
}
