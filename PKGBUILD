...
build() {
    ...
    LIBGL_ALWAYS_SOFTWARE=1 GDK_BACKEND=x11 "$pkgname/tools/xvfb-run.py" -- meson compile -C build
}

check() {
    LIBGL_ALWAYS_SOFTWARE=1 GDK_BACKEND=x11 "$pkgname/tools/xvfb-run.py" -- meson test -C build --print-errorlogs
}
