...
function wait_signal(object, signal) {
    ...
    async activate() {
        ...
        try {
            ...
            for (const heap_dump of this.heap_dumps) {
                if (!dialog)
                    dialog = await this.preferences(); // eslint-disable-line no-await-in-loop

                await wait_frame(dialog); // eslint-disable-line no-await-in-loop
