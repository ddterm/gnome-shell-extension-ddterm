...
Test case combinations are generated by [PICT] from `tests/pict/*.model`
files and stored in `tests/pict/*.gen` files (checked into the repository).

After changing `*.model` files, regenerate the corresponding `*.gen` files:

    meson setup -Dpict=enabled build-dir
    ninja -C build-dir pict

[PICT] is downloaded and built automatically - requires [CMake]
and a C++ compiler (but the option is disabled by default).
